...
export default {
    ...
    * refreshAccessToken({ payload, callBack }, { call, put }) {
      try {
        const params = {
          ...
          refresh_token: localStorage.getItem('PMS_refresh_token'),
          client_id: OA_CLIENT_ID,
          client_secret: OA_CLIENT_SECRET,
          scope: '',
          ...payload,
  ...
  reducers: {
    ...
    saveAccessToken(state, { payload }) {
      localStorage.setItem('PMS_access_token', payload.access_token);
      localStorage.setItem('PMS_access_token_expires_in', new Date().getTime() + ((payload.expires_in - 10) * 1000));
      localStorage.setItem('PMS_refresh_token', payload.refresh_token);
      return {
        ...state,
